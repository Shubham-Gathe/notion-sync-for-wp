...
<div style="margin-top: 10px;">
	<button type="button" class="button" id="add-meta-mapping"><?php esc_html_e( 'Add Row', 'notion-sync-for-wp' ); ?></button>
</div>

<!-- Template for new rows -->
<script type="text/html" id="tmpl-notion-sync-meta-row">
	<tr>
		<td>
			<input type="text" name="notion_sync_mapping[custom_meta][{{INDEX}}][key]" value="" class="regular-text" placeholder="meta_key">
		</td>
		<td>
			<select name="notion_sync_mapping[custom_meta][{{INDEX}}][property]" class="regular-text">
				<option value=""><?php esc_html_e( '-- Select Property --', 'notion-sync-for-wp' ); ?></option>
				<?php foreach ( $properties as $notion_sync_prop ) : ?>
